...
public class UsersController {
    ...
    @GetMapping("/findAll")
    public List<UsersDto> findAllUsers() throws ValidationException{
        log.info("Handling find all users request");
        return usersService.findAll();
    }
    @GetMapping ("/findByLogin")
    public UsersDto findByLogin (@RequestParam String login) throws ValidationException{
        log.info("Handling find by login request: " + login);
        return usersService.findByLogin(login);
    }

    @DeleteMapping("/delete/{id}")
    public ResponseEntity<Void> deleteUsers (@PathVariable Integer id) throws ValidationException{
